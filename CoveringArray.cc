...
CoveringArray::CoveringArray(const SpecificationFile &specificationFile,
							 const ConstraintFile &constraintFile, unsigned long long maxT, int seed, int tabuSize) : satSolver(constraintFile.isEmpty()), specificationFile(specificationFile),
...
void CoveringArray::mostGreedySatRow(std::vector<unsigned> &newLine, const unsigned encode)
{
	...
	for (unsigned column = 0, passing = 0; column < width; ++column)
	{
		...
		while (true)
		{
			...
			{ //1.3 found best, cuculate u(p) for next p
				//break tie randomly
...
void CoveringArray::optimize(unsigned randomSize, unsigned tabustepSize)
{
	...
	while (true)
	{
		...
		tabuSuc = tabuStep(1);
		if(stepIndex > tabustepSize || !tabuSuc)
		{
...
bool CoveringArray::tabuStep(unsigned stepSize)
{
	...
	if (bestRows.size() != 0 /*&& bestScore >= 0*/)
	{
